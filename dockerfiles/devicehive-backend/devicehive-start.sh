...
## By default Backend uses 'ws-kafka-proxy-backend' Spring profile. In case 'rpc-server' profile is used
...
then
    ...
    RPC_PARAMETERS="${RPC_PARAMETERS} -Drpc.server.request-consumer.threads=${DH_RPC_SERVER_REQ_CONS_THREADS:-3}"
    RPC_PARAMETERS="${RPC_PARAMETERS} -Drpc.server.worker.threads=${DH_RPC_SERVER_WORKER_THREADS:-3}"
