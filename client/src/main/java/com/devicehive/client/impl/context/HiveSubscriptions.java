...
import com.devicehive.client.impl.util.SubscriptionTask;
import com.devicehive.client.impl.util.UpdatesSubscriptionTask;
...
import static com.devicehive.client.impl.context.Constants.*;
        ...
        try {
            ...
            if (principal.getUser() != null || principal.getAccessKey() != null)
                for (Pair<String, String> currentSubscription : wsCommandSubscriptionsStorage) {
                    ...
                    WebsocketSubscriptionsUtil.subscribeClientForCommands(hiveContext, timestamp, names,
        ...
        try {
            if (hiveContext.getHivePrincipal().getUser() != null ||
                ...
                for (Pair<String, String> currentSubscription : wsNotificationSubscriptionsStorage) {
                    ...
                    Set<String> names = new HashSet<>();
                    names.add(currentSubscription.getValue());
                    WebsocketSubscriptionsUtil.subscribeClientForNotifications(hiveContext, timestamp, names,
                            currentSubscription.getKey());
                }
        } finally {
            rwWsNotificationsLock.writeLock().unlock();
