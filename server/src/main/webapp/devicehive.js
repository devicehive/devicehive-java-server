    ...
    var
        ...
        formatDate = function (date) {
            if (Object.prototype.toString.call(date) === '[object String]') {
                return date;
            } // already formatted string - do not modify
        ...
        changeChannelState = function (self, newState, oldState) {
            ...
            if (oldState === self.channelState) {
                ...
                $(self).triggerHandler("onChannelStateChanged", [
...
// DeviceHive channel states
    ...
    deviceHive.prototype = {
        ...
        getNotifications: function (deviceId, filter) {
            if (filter && filter.start) {
                filter.start = formatDate(filter.start);
            }
            if (filter && filter.end) {
                filter.end = formatDate(filter.end);
            }
        ...
        openChannel: function (channels) {
            ...
            var deferred = infoQuery.pipe(function (info) {
                ...
                if (!channels) {
                    channels = [];
                    $.each(deviceHive.channels, function (t) {
                        channels.push(t);
                    });
                }
            ...
            return deferred
                .done(function () {
                          changeChannelState(self, deviceHive.channelState.connected);
                      })
                .fail(function () {
                          changeChannelState(self, deviceHive.channelState.disconnected);
                      })
                .promise();
        },
        ...
        subscribe: function (deviceIds) {
            if (deviceIds && !$.isArray(deviceIds)) {
                deviceIds = [deviceIds];
            }
    ...
    deviceHive.channels.longPolling.prototype = {
        ...
        poll: function (timestamp) {
            ...
            if (this.deviceIds) {
                params.deviceGuids = this.deviceIds.join();
            }
