...
public class RedisNotificationService {
    ...
    public Collection<DeviceNotification> getByGuids(final Collection<String> guids, final Timestamp timestamp, final Collection<String> names, final Integer take) {
        ...
        return FluentIterable.from(keys)
                .transform(new Function<String, DeviceNotification>() {
                    @Override
                    public DeviceNotification apply(@Nullable String s) {
                        return get(s, filterByDate, filterByName, timestamp, names);
                    }
                }).filter(new Predicate<DeviceNotification>() {
                    @Override
                    public boolean apply(@Nullable DeviceNotification deviceNotification) {
                        return deviceNotification != null;
                    }
                }).toSortedSet(new DeviceNotificationComparator());
    }
    ...
    public Collection<DeviceNotification> getAll(final Timestamp timestamp, final Collection<String> names, final Integer take) {
        ...
        return FluentIterable.from(keys)
                .transform(new Function<String, DeviceNotification>() {
                    @Override
                    public DeviceNotification apply(String s) {
                        return get(s, filterByDate, filterByName, timestamp, names);
                    }
                }).filter(new Predicate<DeviceNotification>() {
                    @Override
